...
(generated once at import/edit time, see route/models.py's RouteSummary.thumbnail_file_name) or a
completed ride's actual recorded track (see ride/models.py's RideRecord.thumbnail_file_name),
which can differ from the route if the rider stopped early or deviated. Takes plain points rather
than a Route, since that's all either caller has in common. Reuses the same map_assets/map.html as
RideMapView/RouteCreatorMapView in a small window sized to the thumbnail's own dimensions - GTK4
has no off-screen/headless render path for a WebView (GTK3's Gtk.OffscreenWindow was removed), so
the window must actually be presented to render anything, snapshotted, then torn down.
...
def generate(points: Sequence[_HasLatLon], destination_path: Path, on_done: Callable[[bool], None]) -> None:
